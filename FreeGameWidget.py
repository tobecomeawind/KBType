...
class FreeGameWindow(GeneralWindow):
    ...
    def put_language(self, lang):
        ...
        if lang=='eng': self.typeline.load_file(file='levels/eng/ELarge.txt', random=True, max_counter=3)
        else:self.typeline.load_file(file='levels/ru/RLarge.txt', random=True, max_counter=3)
        self.stack_widgets.setCurrentIndex(0)
